...
from .hookenv import charm_name
from .hookenv import local_unit

UPDATEDB_PATH = '/etc/updatedb.conf'
CA_CERT_DIR = '/usr/local/share/ca-certificates'

log = logging.getLogger("lib.extensions.core.host")
...
def _add_new_group(group_name, system_group=False, gid=None):
    cmd = ['addgroup']
    if gid:
...
def add_group(group_name, system_group=False, gid=None):
    ...
    try:
        ...
        _add_new_group(group_name, system_group, gid)
        group_info = grp.getgrnam(group_name)
    return group_info
...
def write_file(path, content, owner='root', group='root', perms=0o444):
    ...
    if content != existing_content:
        log.debug("Writing file {} {}:{} {:o}".format(path, owner, group, perms))
    ...
    if existing_uid != uid:
        log.debug("Changing uid on already existing content: {} -> {}".format(existing_uid, uid))
        os.chown(path, uid, -1)
    if existing_gid != gid:
        log.debug("Changing gid on already existing content: {} -> {}".format(existing_gid, gid))
        os.chown(path, -1, gid)
    if existing_perms != perms:
        log.debug("Changing permissions on existing content: {} -> {}".format(existing_perms, perms))
        os.chmod(path, perms)
...
def mounts():
    ...
    with open('/proc/mounts') as f:
        ...
        system_mounts = [m[1::-1] for m in [line.strip().split() for line in f.readlines()]]
...
def pwgen(length=None):
    ...
    alphanumeric_chars = [
        symbol for symbol in (string.ascii_letters + string.digits)
        if symbol not in '0QDvOo']
...
def install_ca_cert(ca_cert, name=None):
    ...
    if file_hash(cert_file) == new_hash:
        return
    log.info("Installing new CA cert at: {}".format(cert_file))
